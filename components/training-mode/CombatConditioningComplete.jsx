...
// Combat Conditioning complete — design 8a layout (shared complete-screen design).
const GOLD = '#fde047';
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', minHeight: '100dvh' }}>
        <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '18px 16px calc(24px + env(safe-area-inset-bottom,0px))' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            ...
            <div style={{ font: "900 20px 'Orbitron',sans-serif", color: '#fff', letterSpacing: '0.04em' }}>{mission.missionName || 'COMBAT CONDITIONING'}</div>
            <div style={{ font: "600 10px 'Rajdhani',sans-serif", color: '#c4a4d8', marginTop: 2 }}>{mission.style} · {mission.difficulty} · {mission.estimatedMinutes || '--'} min</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 14 }}>
            ...
            <Stat value={String(drillsCompleted)} label="DRILLS" color="#fff"/>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            <button onClick={onNewMission} style={{ width: '100%', height: 50, border: 'none', borderRadius: 12, background: 'linear-gradient(135deg,#fde047,#f59e0b)', color: '#0a0014', font: "900 13px 'Orbitron',sans-serif", letterSpacing: '0.08em', cursor: 'pointer', boxShadow: '0 0 20px rgba(253,224,71,0.35)' }}>NEW COMBAT MISSION</button>
